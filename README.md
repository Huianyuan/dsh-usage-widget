...
- 🖱️ **自由拖拽**：按住挂件可拖到任意位置（Pointer 事件，鼠标/触屏通用；钳制在视口内不会拖丢），位置记忆于浏览器 localStorage，刷新后自动恢复
- 🔑 **key 多级解析**：DSH 凭据优先 → 本地配置文件回退，**零配置开箱即用**
- 🧩 **订阅源注册表**：代码模块注册，自动发现（无 key 的源自动隐藏）、每源独立缓存与错误隔离
- 🎯 **按提供商自动切换**：识别当前会话使用的 provider（`assistant/message` / `request/context` 事件 + 监听 `settings.yaml` 的模型切换），**SSE 约 1 秒内**只显示对应订阅卡片；provider 未匹配时显示全部并给出提示行；上次选择持久化记忆，重启后直接沿用
