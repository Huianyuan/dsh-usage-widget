...
// 从 WIDGET_JS 模板字符串中按配平花括号提取 formatCountdown 函数（独立于浏览器执行）
function extractFormatCountdown() {
  const marker = 'function formatCountdown'
  const start = WIDGET_JS.indexOf(marker)
  assert.ok(start >= 0, 'formatCountdown 未在 WIDGET_JS 中找到')
  ...
  assert.ok(end > 0, 'formatCountdown 花括号未配平')
  const chunk = WIDGET_JS.slice(start, end + 1)
  return new Function('return ' + chunk)()
}

const formatCountdown = extractFormatCountdown()
