...
function savePos(x, y) {
  try { localStorage.setItem(POS_KEY, JSON.stringify({ x: Math.round(x), y: Math.round(y) })) } catch (err) {}
}
function restorePos() {
  try {
    ...
    if (typeof p.x !== 'number' || typeof p.y !== 'number' || p.x < 0 || p.y < 0) return false
    var w = root.offsetWidth || 280
    var h = root.offsetHeight || 100
    if (p.x + w > window.innerWidth || p.y + h > window.innerHeight) return false // 视口变化后丢弃旧位置
    applyPos(p.x, p.y)
    return true
...
function initPos() {
  ...
  var w = root.offsetWidth || 280
  var h = root.offsetHeight || 100
  applyPos(Math.max(0, window.innerWidth - w - 12), Math.max(0, window.innerHeight - h - 12))
}
...
function endDrag() {
  ...
  if (moved) {
    ...
    savePos(parseFloat(root.style.left) || root.offsetLeft, parseFloat(root.style.top) || root.offsetTop)
