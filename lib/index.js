...
const DEEPSEEK = {
  ...
  name: 'DeepSeek',
  metric: 'amount',
  providerHints: ['deepseek', 'deepseek-api'],
...
export function selectSourcesByProvider(viewDescs, sources, provider) {
  ...
  const matched = sources.filter((s) => (s.providerHints || []).includes(provider)).map((s) => s.id)
...
function apply(ctx) {
  ...
  const disposers = []
  let activeProvider = null // 最近一次 assistant 消息的提供商（provider 路由 id）

  disposers.push(ctx.on('session/event', (session, event) => {
    try {
      if (!event || event.type !== 'assistant/message') return
      const src = event.data && event.data.message && event.data.message.source
      if (src && src.kind === 'model' && typeof src.provider === 'string' && src.provider) {
        activeProvider = src.provider
      }
    } catch {}
  }))
