...
var css = [
  ...
  '.ocuw-card{width:270px;height:270px;box-sizing:border-box;background:rgba(255,255,255,.94);border:1px solid rgba(31,38,82,.28);border-radius:12px;padding:10px 12px;box-shadow:0 6px 18px rgba(0,0,0,.16);cursor:pointer;position:relative;transform-origin:50% 50%;transition:transform .22s cubic-bezier(.34,1.56,.64,1)}',
  ...
  '.ocuw-msg{width:270px;box-sizing:border-box;background:rgba(255,255,255,.94);border:1px solid rgba(31,38,82,.28);border-radius:12px;padding:12px 14px;color:#8a93b8;font-size:12px;text-align:center;cursor:pointer}',
  '.ocuw-hint{width:270px;box-sizing:border-box;font-size:10px;color:#8a93b8;text-align:center;border-top:1px dashed rgba(31,38,82,.15);padding-top:4px}'
].join('\\n')
...
function restorePos() {
  try {
    ...
    if (typeof p.px === 'number' && typeof p.py === 'number') {
      ...
      px = pctFromPx(p.x, root.offsetWidth || 270, window.innerWidth)
...
root.addEventListener('pointermove', function (e) {
  ...
  var c = clampDrag(dragState.ox + dx, dragState.oy + dy, root.offsetWidth || 270, root.offsetHeight || 100, window.innerWidth, window.innerHeight)
  applyPos(c.x, c.y)
})
function endDrag() {
  if (!dragState) return
