...
function restorePos() {
  try {
    ...
    if (typeof p.px === 'number' && typeof p.py === 'number') {
      ...
      px = pctFromPx(p.x, root.offsetWidth || 280, window.innerWidth)
...
root.addEventListener('pointermove', function (e) {
  ...
  var c = clampDrag(dragState.ox + dx, dragState.oy + dy, root.offsetWidth || 280, root.offsetHeight || 100, window.innerWidth, window.innerHeight)
  applyPos(c.x, c.y)
})
function endDrag() {
  if (!dragState) return
