...
var css = [
  '.ocuw-root{position:fixed;right:12px;bottom:12px;z-index:9999;display:flex;flex-direction:column;gap:10px;font-family:inherit;user-select:none;-webkit-user-select:none;color-scheme:light;touch-action:none;cursor:grab}',
  '.ocuw-root.ocuw-dragging{cursor:grabbing}',
  '.ocuw-root.ocuw-dragging *{pointer-events:none}',
  '.ocuw-card{width:275px;box-sizing:border-box;background:rgba(255,255,255,.94);border:1px solid rgba(31,38,82,.28);border-radius:12px;padding:10px 12px;box-shadow:0 6px 18px rgba(0,0,0,.16);cursor:pointer;position:relative;transform-origin:50% 50%;transition:transform .22s cubic-bezier(.34,1.56,.64,1)}',
  ...
  '.ocuw-msg{width:275px;box-sizing:border-box;background:rgba(255,255,255,.94);border:1px solid rgba(31,38,82,.28);border-radius:12px;padding:12px 14px;color:#8a93b8;font-size:12px;text-align:center;cursor:pointer}',
  '.ocuw-hint{width:275px;box-sizing:border-box;font-size:10px;color:#8a93b8;text-align:center;border-top:1px dashed rgba(31,38,82,.15);padding-top:4px}'
].join('\\n')
...
// ---- 自由拖拽（Pointer 事件：鼠标+触屏；位置记忆到 localStorage） ----
function clampDrag(x, y, w, h, vw, vh) {
  var nx = Math.max(0, Math.min(x, Math.max(0, vw - w)))
  var ny = Math.max(0, Math.min(y, Math.max(0, vh - h)))
  return { x: nx, y: ny }
}
var dragState = null
var suppressClick = false
var posPct = null // 当前位置的「视口比例」（浏览器缩放/窗口变化时保持相对位置）
var POS_KEY = 'ocuw-pos'
// 像素 ↔ 视口比例：span = vw - w（挂件占据后的可用跨度）
function pctFromPx(x, w, vw) {
  var span = vw - w
  if (span <= 0) return 0
  return Math.max(0, Math.min(1, x / span))
}
function pxFromPct(p, w, vw) {
  var span = vw - w
  if (span <= 0) return 0
  return Math.max(0, Math.min(span, p * span))
}
...
function savePosPct() {
  var w = root.offsetWidth || 275
  var h = root.offsetHeight || 100
  posPct = {
    x: pctFromPx(parseFloat(root.style.left) || root.offsetLeft, w, window.innerWidth),
    y: pctFromPx(parseFloat(root.style.top) || root.offsetTop, h, window.innerHeight),
  }
  try { localStorage.setItem(POS_KEY, JSON.stringify({ px: posPct.x, py: posPct.y })) } catch (err) {}
}
function restorePos() {
  try {
    ...
    if (typeof p.px === 'number' && typeof p.py === 'number') {
      ...
      // 旧版绝对像素：迁移为比例（兼容已存的位置）
      px = pctFromPx(p.x, root.offsetWidth || 275, window.innerWidth)
      py = pctFromPx(p.y, root.offsetHeight || 100, window.innerHeight)
    } else return false
    posPct = { x: Math.max(0, Math.min(1, px)), y: Math.max(0, Math.min(1, py)) }
    reposition()
    return true
  } catch (err) { return false }
}
function reposition() {
  if (!posPct || dragState) return // 拖拽中不打扰
  var w = root.offsetWidth || 275
  var h = root.offsetHeight || 100
  applyPos(pxFromPct(posPct.x, w, window.innerWidth), pxFromPct(posPct.y, h, window.innerHeight))
}
function initPos() {
  if (restorePos()) return
  var w = root.offsetWidth || 275
  var h = root.offsetHeight || 100
  // 默认右下角 12px → 存成比例，之后任何缩放都保持在右下相对位置
  posPct = {
    x: pctFromPx(Math.max(0, window.innerWidth - w - 12), w, window.innerWidth),
    y: pctFromPx(Math.max(0, window.innerHeight - h - 12), h, window.innerHeight),
  }
  reposition()
}
window.addEventListener('resize', reposition) // 浏览器放大/缩小或窗口变化：按比例重放位置
root.addEventListener('pointerdown', function (e) {
  if (e.button !== undefined && e.button !== 0) return // 仅左键/触摸按下开始拖
  dragState = { sx: e.clientX, sy: e.clientY, ox: root.offsetLeft, oy: root.offsetTop, moved: false }
  root.classList.add('ocuw-dragging')
  try { if (root.setPointerCapture) root.setPointerCapture(e.pointerId) } catch (err) {}
  var el = e.target
...
root.addEventListener('pointermove', function (e) {
  ...
  var c = clampDrag(dragState.ox + dx, dragState.oy + dy, root.offsetWidth || 275, root.offsetHeight || 100, window.innerWidth, window.innerHeight)
  applyPos(c.x, c.y)
})
function endDrag() {
  if (!dragState) return
  ...
  dragState = null
  root.classList.remove('ocuw-dragging')
  clearPress()
  if (moved) {
    suppressClick = true // 拖完不触发点击刷新
    savePosPct()
